...
private fun IntRange.containsRange(other: IntRange) = other.first >= first && other.last <= last
...
fun parseAssignments(assignmentPair: String) = assignmentPair.split(",").map { range ->
    range.split("-").map {
        it.toInt()
    }.let {
        it[0]..it[1]
...
fun overlaps(ranges: Pair<IntRange, IntRange>) = ranges.first.overlaps(ranges.second)
